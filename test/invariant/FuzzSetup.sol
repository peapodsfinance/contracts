...
contract FuzzSetup is Test, FuzzBase {
  ...
  address internal circuitBreaker =
    vm.addr(uint256(keccak256('circuitBreaker')));
  ...
  function _deployUniV2() internal {
    ...
    _v2SwapRouter = new UniswapV2Router02(
  ...
  function _deployUniV3() internal {
    ...
    _v3peasDaiPool = UniswapV3Pool(
      _uniV3Factory.createPool(address(_peas), address(_mockDai), 10000)
    );
    ...
    _v3wethDaiPool = UniswapV3Pool(
      _uniV3Factory.createPool(address(_weth), address(_mockDai), 10000)
    );
    ...
    _v3wethDaiFlash = UniswapV3Pool(
      _uniV3Factory.createPool(address(_weth), address(_mockDai), 500)
    );
    ...
    _v3peasDaiFlash = UniswapV3Pool(
      _uniV3Factory.createPool(address(_peas), address(_mockDai), 500)
    );
    ...
    _v3SwapRouter = new SwapRouter02(
      address(_uniV2Factory),
  ...
  function _deployProtocolFees() internal {
    ...
    if (address(this) == 0x7FA9385bE102ac3EAc297483Dd6233D62b3e1496) {
      ...
      _protocolFeeRouter = ProtocolFeeRouter(
  ...
  function _deployRewardsWhitelist() internal {
    if (address(this) == 0x7FA9385bE102ac3EAc297483Dd6233D62b3e1496) {
      ...
      _rewardsWhitelist = RewardsWhitelist(
  ...
  function _deployTwapUtils() internal {
    if (address(this) == 0x7FA9385bE102ac3EAc297483Dd6233D62b3e1496) {
      ...
      _twapUtils = MockV3TwapUtilities(
  ...
  function _deployDexAdapter() internal {
    _dexAdapter = new UniswapDexAdapter(
      _twapUtils,
  ...
  function _deployFraxPairs() internal {
    ...
    // deposit some asset
    IERC20(_pod1Weth.PAIRED_LP_TOKEN()).approve(
      address(_fraxLPToken1Weth),
      type(uint256).max
    );
    ...
    // deposit some asset
    IERC20(_pod2.PAIRED_LP_TOKEN()).approve(
      address(_fraxLPToken2),
      type(uint256).max
    );
    ...
    // deposit some asset
    IERC20(_pod4.PAIRED_LP_TOKEN()).approve(
      address(_fraxLPToken4),
      type(uint256).max
    );
  ...
  function _deployLendingAssetVault() internal {
    ...
    vaultAsset1Peas.approve(
      address(_fraxLPToken1Peas),
      vaultAsset1Peas.totalSupply()
    );
    vaultAsset1Peas.approve(
    ...
    vm.prank(timelock);
    _fraxLPToken1Peas.setExternalAssetVault(
      IERC4626Extended(address(_lendingAssetVault))
    );

    IERC20 vaultAsset1Weth = IERC20(_fraxLPToken1Weth.asset());
    vaultAsset1Weth.approve(
      address(_fraxLPToken1Weth),
      vaultAsset1Weth.totalSupply()
    );
    vaultAsset1Weth.approve(
      address(_lendingAssetVault),
      vaultAsset1Weth.totalSupply()
    ...
    vm.prank(timelock);
    _fraxLPToken1Weth.setExternalAssetVault(
      IERC4626Extended(address(_lendingAssetVault))
    );
    ...
    // set external access vault
    vm.prank(timelock);
    _fraxLPToken2.setExternalAssetVault(
      IERC4626Extended(address(_lendingAssetVault))
    );
    ...
    // set external access vault
    vm.prank(timelock);
    _fraxLPToken4.setExternalAssetVault(
      IERC4626Extended(address(_lendingAssetVault))
    );
  ...
  function _deployBalancerVault() internal {
    ...
    if (address(this) == 0x7FA9385bE102ac3EAc297483Dd6233D62b3e1496) {
      vm.etch(
        ...
        _bytecode.BALANCER_VAULT_BYTECODE()
      );
  ...
  function _deployLeverageManager() internal {
    ...
    _leverageManager.setLendingPair(
      address(_pod1Peas),
      address(_fraxLPToken1Peas)
    );
    _leverageManager.setLendingPair(
  ...
  function _deployFlashSources() internal {
    ...
    _leverageManager.setFlashSource(
      address(_pod1Peas.PAIRED_LP_TOKEN()),
      address(_uniswapV3FlashSourcePeas)
    );
    _leverageManager.setFlashSource(
      address(_pod1Weth.PAIRED_LP_TOKEN()),
      address(_uniswapV3FlashSourceWeth)
    );
    _leverageManager.setFlashSource(
      address(_pod2.PAIRED_LP_TOKEN()),
      address(_uniswapV3FlashSourcePeas)
    );
    _leverageManager.setFlashSource(
      address(_pod4.PAIRED_LP_TOKEN()),
      address(_uniswapV3FlashSourceWeth)
    );
  }
  ...
  function randomIndexToken(
    WeightedIndex pod,
    uint256 seed
  ) internal view returns (address) {
    IDecentralizedIndex.IndexAssetInfo[] memory indexTokens = pod
      .getAllAssets();
    return indexTokens[bound(seed, 0, indexTokens.length - 1)].token;
  }

  function randomAspTKN(
    uint256 seed
  ) internal view returns (AutoCompoundingPodLp) {
  ...
  function _checkTokenBalances(
    ...
    address user,
    uint256 amount
  ) internal returns (bool hasEnough) {
    IDecentralizedIndex.IndexAssetInfo[] memory indexTokens = pod
      .getAllAssets();
