...
contract LendingAssetVaultHandler is Properties {
  ...
  function lendingAssetVault_deposit(
    ...
    amount = fl.clamp(
      amount,
      0,
      IERC20(cache.vaultAsset).balanceOf(cache.user)
    );
    ...
    // ACTION
    vm.prank(cache.user);
    try _lendingAssetVault.deposit(amount, cache.receiver) returns (
  ...
  function lendingAssetVault_mint(
    ...
    shares = fl.clamp(
      shares,
      0,
      _lendingAssetVault.convertToShares(
        IERC20(cache.vaultAsset).balanceOf(cache.user)
      )
    );
    cache.assets = _lendingAssetVault.convertToAssets(shares);
    if (
      cache.assets == 0 || _lendingAssetVault.convertToShares(cache.assets) == 0
    ) return;
    ...
    // ACTION
    vm.prank(cache.user);
    try _lendingAssetVault.mint(shares, cache.receiver) returns (
  ...
  function lendingAssetVault_withdraw(
    ...
    vm.prank(cache.user);
    try
      _lendingAssetVault.withdraw(assets, cache.receiver, cache.user)
  ...
  function lendingAssetVault_redeem(
    ...
    cache.assets = _lendingAssetVault.convertToAssets(shares);
    cache.maxAssets = _lendingAssetVault.convertToAssets(
      _lendingAssetVault.maxRedeem(cache.user)
    );
    ...
    // ACTION
    vm.prank(cache.user);
    try _lendingAssetVault.redeem(shares, cache.receiver, cache.user) returns (
  ...
  function lendingAssetVault_redeemFromVault(
    ...
    (cache.assetShares, , ) = cache.lendingPair.getUserSnapshot(
    ...
    cache.assets = shares == 0
      ? cache.lendingPair.convertToAssets(
        cache.lendingPair.balanceOf(address(_lendingAssetVault))
      )
      : cache.lendingPair.convertToAssets(shares);

    if (
      cache.assets >
      IERC20(cache.lendingPairAsset).balanceOf(address(cache.lendingPair))
    ) return;
