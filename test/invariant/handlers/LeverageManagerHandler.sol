...
contract LeverageManagerHandler is Properties {
  ...
  function leverageManager_initializePosition(
    ...
    // ACTION
    try
      _leverageManager.initializePosition(
        address(cache.pod),
        cache.user,
  ...
  function leverageManager_addLeverage(
    ...
    cache.positionId = fl.clamp(
      positionIdSeed,
      0,
      cache.positionNFT.totalSupply()
    );
    ...
    cache.aspTKN = AutoCompoundingPodLp(
    ...
    address lpPair = _uniV2Factory.getPair(
      cache.podAddress,
      cache.pod.PAIRED_LP_TOKEN()
    );
    (uint112 reserve0, uint112 reserve1, ) = IUniswapV2Pair(lpPair)
      .getReserves();
    ...
    if (
      pairedLpAmount >
      IERC20(cache.pod.PAIRED_LP_TOKEN()).balanceOf(
        IFlashLoanSource(cache.flashSource).source()
      )
    ) return;
    ...
    (uint256 fraxAssets, , uint256 fraxBorrows, , ) = IFraxlendPair(
      cache.lendingPair
    ).getPairAccounting();
    ...
    {
      uint256 sharesReturned = IFraxlendPair(cache.lendingPair)
        .userBorrowShares(cache.custodian);
      uint256 _borrowAssetssToRepay = VaultAccountingLibrary.toAmount(
        IFraxlendPair(cache.lendingPair).totalBorrow(),
        sharesReturned,
        false
      );
      fl.log('USER ASSET BALANCE', _borrowAssetssToRepay);
  ...
  function leverageManager_removeLeverage(
    ...
    (cache.interestEarned, , , , , ) = FraxlendPair(cache.lendingPair)
      .previewAddInterest();

    // borrowAssets starts as shares, will change to assets here in a sec
    borrowAssets = fl.clamp(
      borrowAssets,
      0,
    ...
    borrowAssets = VaultAccountingLibrary.toAmount(
      IFraxlendPair(cache.lendingPair).totalBorrow(),
      borrowAssets + cache.interestEarned,
      false
    );
    fl.log('BORROWASSETS', borrowAssets);
    uint256 feeAmount = FullMath.mulDivRoundingUp(borrowAssets, 10000, 1e6);

    collateralAmount = fl.clamp(
      collateralAmount,
      0,
      IFraxlendPair(cache.lendingPair).userCollateralBalance(cache.custodian)
    );
    ...
    if (
      !_solventCheckAfterRepay(
        ...
        IFraxlendPair(cache.lendingPair).userBorrowShares(cache.custodian),
        cache.repayShares,
        IFraxlendPair(cache.lendingPair).userCollateralBalance(
          cache.custodian
        ) - borrowAssets
      )
    ) return;
    ...
    vm.prank(cache.user);
    IERC20(cache.borrowToken).approve(
    ...
    vm.prank(cache.user);
    try
      _leverageManager.removeLeverage(
        cache.positionId,
        borrowAssets,
        borrowAssets,
        0,
  ...
  function _solventCheckAfterRepay(
    ...
    (, , , , uint256 highExchangeRate) = FraxlendPair(lendingPair)
      .exchangeRateInfo();
    ...
    isSolvent = true;
    uint256 _ltv = (((sharesAfterRepay * highExchangeRate) /
      FraxlendPair(lendingPair).EXCHANGE_PRECISION()) *
