...
contract TokenRewardsExposed is TokenRewards {
  ...
  function exposedCumulativeRewards(
    address _token,
    uint256 _share,
    bool _roundUp
...
contract TokenRewardsTest is Test {
  ...
  function setRewardsAndCalculate(
    uint256 share,
    ...
    uint256 result = tokenRewards.exposedCumulativeRewards(
      token,
      share,
      roundUp
    );
    uint256 expected = calculateExpectedRewards(
      share,
      rewardsPerShare,
      roundUp
    );
