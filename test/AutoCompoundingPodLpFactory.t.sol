...
contract AutoCompoundingPodLpFactoryTest is Test {
  ...
  function setUp() public {
    ...
    stakingPoolToken = new MockStakingPoolToken(
      'Staking Pool Token',
      'SPT',
      address(tokenRewards)
    );
    pod = new MockDecentralizedIndex(
      address(rewardsToken),
      address(stakingPoolToken)
    );
  ...
  function testCreateWithMinimumDeposit() public {
    ...
    assertTrue(
      address(podLp) != address(0),
      'AutoCompoundingPodLp was not created'
    );
    ...
    assertEq(
      podLp.symbol(),
      symbol,
      'AutoCompoundingPodLp symbol is incorrect'
    );
    ...
    assertEq(
      podLp.owner(),
      owner,
      'AutoCompoundingPodLp ownership was not transferred correctly'
    );
