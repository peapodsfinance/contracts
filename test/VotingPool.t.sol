...
contract VotingPoolTest is Test {
  ...
  function setUp() public {
    ...
    votingPool.addOrUpdateAsset(
      address(pairedLpToken),
      IStakingConversionFactor(address(0)),
      true
    );
  }
  ...
  function testStake() public {
    ...
    assertEq(
      votingPool.balanceOf(alice),
      stakeAmount,
      'Staked balance should match'
    );
    assertEq(
      pairedLpToken.balanceOf(alice),
      INITIAL_BALANCE - stakeAmount,
      'Token balance should decrease'
    );
    assertEq(
      pairedLpToken.balanceOf(address(votingPool)),
      stakeAmount,
      'VotingPool should hold staked tokens'
    );
  }
  ...
  function testMultipleUsersStakeUnstake() public {
    ...
    assertEq(
      votingPool.balanceOf(alice),
      aliceStake,
      "Alice's staked balance should match"
    );
    assertEq(
      votingPool.balanceOf(bob),
      bobStake,
      "Bob's staked balance should match"
    );
    ...
    assertEq(
      votingPool.balanceOf(alice),
      0,
      "Alice's staked balance should be zero"
    );
    assertEq(
      votingPool.balanceOf(bob),
      0,
      "Bob's staked balance should be zero"
    );
    assertEq(
      pairedLpToken.balanceOf(address(votingPool)),
      0,
      'VotingPool balance should be zero'
    );
  }
}
...
contract MockV3TwapUtilities {
  function sqrtPriceX96FromPoolAndInterval(
    address
  ) external pure virtual returns (uint160) {
    return 0;
  }
