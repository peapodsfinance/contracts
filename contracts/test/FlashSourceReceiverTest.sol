...
contract FlashSourceReceiverTest is IFlashLoanRecipient {
  ...
  function tryFlash(
    ...
    if (_extraPayment > 0) {
      ...
      IERC20(_extraToken).safeTransferFrom(
        msg.sender,
        address(this),
        _extraPayment
      );
      IERC20(_extraToken).safeIncreaseAllowance(
        address(_source),
        _extraPayment
      );
    }
  ...
  function callback(bytes memory _data) external override {
    IFlashLoanSource.FlashData memory _parsedData = abi.decode(
      _data,
    ...
    IERC20(_parsedData.token).safeTransfer(
      IFlashLoanSource(msg.sender).source(),
      _parsedData.amount + _parsedData.fee
    );
  }
}
