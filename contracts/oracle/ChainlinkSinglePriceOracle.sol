...
contract ChainlinkSinglePriceOracle is IMinimalSinglePriceOracle, Ownable {
  ...
  function getPriceUSD18(
    ...
    bool _quoteIsBad;
    uint256 _maxDelayQuote = feedMaxOracleDelay[_priceFeedQuote] > 0
      ? feedMaxOracleDelay[_priceFeedQuote]
      : defaultMaxOracleDelay;
    uint256 _isBadTimeQuote = block.timestamp - _maxDelayQuote;
    (_price18, _quoteUpdatedAt, _quoteIsBad) = _getChainlinkPriceFeedPrice18(
      _priceFeedQuote
    );
    _isBadData = _quoteIsBad || _quoteUpdatedAt < _isBadTimeQuote;

    uint256 _maxDelayBase = feedMaxOracleDelay[_priceFeedBase] > 0
      ? feedMaxOracleDelay[_priceFeedBase]
      : defaultMaxOracleDelay;
    ...
    returns (uint256 _price18, uint256 _updatedAt, bool _isBadAnswer)
  {
    ...
    (, int256 _price, , uint256 _lastUpdated, ) = AggregatorV2V3Interface(
      _priceFeed
    ).latestRoundData();
    _isBadAnswer = !_isValidAnswer(_priceFeed, _price);
  ...
  function _isValidAnswer(
    address _feed,
    int256 _answer
  ) internal view returns (bool _isValid) {
    _isValid = true;
    int192 _min = IOffchainAggregator(IEACAggregatorProxy(_feed).aggregator())
      .minAnswer();
    int192 _max = IOffchainAggregator(IEACAggregatorProxy(_feed).aggregator())
      .maxAnswer();
  ...
  function setFeedMaxOracleDelay(
    address _feed,
