...
contract TokenBridge is ICCIPTokenBridge, CCIPReceiver, Context {
  ...
  constructor(
    ...
    ICCIPTokenRouter _tokenRouter
  ) CCIPReceiver(address(_ccipRouter)) {
  ...
  function bridgeTokens(
    ...
    ICCIPTokenRouter.TokenConfig memory _bridgeConf = tokenRouter.getConfig(
      _token,
      _chainSelector
    );
    ...
    uint256 _amountActual = _processInboundTokens(
      ...
      _msgSender(),
      _amount,
      _bridgeConf.sourceTokenMintBurn
    );
    (
    ...
    _messageId = ccipRouter.ccipSend{ value: _nativeFees }(
      _chainSelector,
      _evm2AnyMessage
    );
    emit TokensSent(
  ...
  function _getMessageFee(
    ...
    return (
      _evm2AnyMessage,
      ccipRouter.getFee(_bridgeConf.targetChain, _evm2AnyMessage)
    );
  }
  ...
  function _buildMsg(
    ...
    Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
      ...
      extraArgs: Client._argsToBytes(
        Client.EVMExtraArgsV1({ gasLimit: tokenRouter.targetChainGasLimit() })
      ),
  ...
  function _processInboundTokens(
    address _token,
    address _user,
    uint256 _amount,
    bool _isMintBurn
